...
#   7. a read killed mid-fetch leaves no `refs/board/evidence/<pid>` behind.
...
leaked="$(git -C "$tick" for-each-ref --format='%(refname)' 'refs/board/evidence/*')"
...
#
# The leak window is between the fetch that creates `refs/board/evidence/<pid>`
...
# touch `refs/board/`. A leaked ref is permanent and pins everything its fetch
...
for _ in $(seq 1 100); do
  git -C "$tick" show-ref --verify --quiet "refs/board/evidence/$victim" && break
  sleep 0.1
done
git -C "$tick" show-ref --verify --quiet "refs/board/evidence/$victim" || {
