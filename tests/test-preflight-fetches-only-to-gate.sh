...
# `ops/deploy-mango.sh` in a checkout 169 commits behind, merged, and broke the
# mango deploy for half an hour. It never fixed that and could not: it measures
# against `refs/remotes/origin/main`, which is the very ref the tick misread.
