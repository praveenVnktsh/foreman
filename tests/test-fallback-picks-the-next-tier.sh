...
# shell that runs this test cannot decide what it proves.
set -euo pipefail
...
# Runs fallback.py under the environment config.sh would export for a claude
...
# config.sh exports every stage model. An unset one means config.sh was never
# sourced, and reading it as empty would spawn the stage on inherit.
