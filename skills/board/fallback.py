...
WHAT IT READS. The environment only, as `skills/board/config.sh` exports it:
FOREMAN_HOME, FALLBACK_TIERS, FALLBACK_COOLDOWN_MINUTES, and each stage's
*_MODEL and *_FLOOR. bin/installation.py is the one reader of the [fallback]
table in installation.toml. A second parser here would be a copy that drifts.
The cleanup stage pairs CLEANUP_MODEL with PLAN_FLOOR, because a cleanup pass is
the plan's work in miniature; config.sh defaults CLEANUP_MODEL for that reason.

AN UNSET VARIABLE IS REFUSED. AN EMPTY ONE IS A VALUE. config.sh exports every
one of them, and installation.py always emits them, so an unset one means this
process did not start under config.sh. Resolving anyway would answer "" for the
stage model, which the CLI reads as inherit: every stage spawned on whatever
the caller runs on, with exit code 0. dispatch.sh reads a refusal as "spawn on
the first choice and warn", so a broken helper costs the fallback and never the
dispatch. An empty value keeps its meaning: `PLAN_MODEL=` is inherit, and
`FALLBACK_TIERS=` turns fallback off.
...
def required(name: str) -> str:
    ...
    if value is None:
        die(f"{name} is unset; expected it exported by skills/board/config.sh "
